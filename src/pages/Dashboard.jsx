...
function ImportBar({ lastImportAt, isAdmin }) {
      ...
      if (data.ok) {
        try {
          ...
          if (settingsList.length > 0) {
            await base44.entities.Settings.update(settingsList[0].id, { last_import_at: now });
          } else {
            await base44.entities.Settings.create({ last_import_at: now });
          }
        } catch (e) {}
  ...
  const bgColors = { ok: 'bg-white', yellow: 'bg-[#fef9c3]', red: 'bg-[#fee2e2]' };
  const borderColors = { ok: 'border-slate-200', yellow: 'border-yellow-300', red: 'border-red-300' };
  ...
  return (
    <div className={`mb-6 px-5 py-3 rounded-xl border ${bgColors[severity]} ${borderColors[severity]}`} dir="rtl">
      <div className="flex flex-row items-center justify-between gap-4">
        <div className="flex-1 text-right">
          ...
          {hoursSince !== null && hoursSince >= 48 && !noDate && (
            <div className="mt-0.5 text-sm text-red-700 font-medium">הנתונים לא עודכנו ב־48 השעות האחרונות – מומלץ לבצע ייבוא</div>
          )}
          {hoursSince !== null && hoursSince >= 24 && hoursSince < 48 && !noDate && (
            <div className="mt-0.5 text-sm text-yellow-800 font-medium">הנתונים לא עודכנו ב־24 השעות האחרונות – מומלץ לבצע ייבוא</div>
          )}
          {noDate && (
            <div className="mt-0.5 text-sm text-red-700 font-semibold">נדרש לייבא נתונים מעדכניים</div>
          )}
          {syncError && (
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {isAdmin && showWarning && (
            <Button onClick={() => navigate(createPageUrl('Import'))}
    ...
    Promise.all(
      missing.map((req) =>
        base44.entities.Status.create({
          name: req.name, type: 'LEGAL', color: req.color, is_active: true, is_default: false,
        })
      )
    ).then(() => {
      refetchStatuses();
    ...
    <TooltipProvider>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50 overflow-x-hidden" dir="rtl">
          ...
          <div className="-mt-2 md:-mt-4 px-3 md:px-[26px] pb-4 md:pb-6 relative z-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4">
              <div className="kpi-card-glow min-h-[80px] md:min-h-[108px] rounded-[16px] md:rounded-[20px] bg-[rgba(255,255,255,0.90)] backdrop-blur-[12px] border border-[rgba(225,231,248,0.96)] shadow-[0_12px_30px_rgba(126,145,220,0.12),inset_0_1px_0_rgba(255,255,255,0.96)] p-3 md:p-4 flex flex-col justify-between">
                ...
                <div><p className="text-[24px] md:text-[32px] font-black leading-none text-[#2bc9a8]">{new Intl.NumberFormat('he-IL', { notation: 'compact', maximumFractionDigits: 0 }).format(records.reduce((sum, r) => sum + (r.totalDebt || 0), 0))}</p></div>
              </div>
              <div className="kpi-card-glow min-h-[80px] md:min-h-[108px] rounded-[16px] md:rounded-[20px] bg-[rgba(255,255,255,0.90)] backdrop-blur-[12px] border border-[rgba(225,231,248,0.96)] shadow-[0_12px_30px_rgba(126,145,220,0.12),inset_0_1px_0_rgba(255,255,255,0.96)] p-3 md:p-4 flex flex-col justify-between">
                <p className="text-[14px] md:text-[18px] font-black uppercase" style={{color: '#1d5bbd'}}>דמי ניהול</p>
                <div><p className="text-[24px] md:text-[32px] font-black leading-none text-[#6270ff]">{new Intl.NumberFormat('he-IL', { notation: 'compact', maximumFractionDigits: 0 }).format(records.reduce((sum, r) => sum + (r.monthlyDebt || 0), 0))}</p></div>
              </div>
              <div className="kpi-card-glow min-h-[80px] md:min-h-[108px] rounded-[16px] md:rounded-[20px] bg-[rgba(255,255,255,0.90)] backdrop-blur-[12px] border border-[rgba(225,231,248,0.96)] shadow-[0_12px_30px_rgba(126,145,220,0.12),inset_0_1px_0_rgba(255,255,255,0.96)] p-3 md:p-4 flex flex-col justify-between">
                <p className="text-[14px] md:text-[18px] font-black uppercase" style={{color: '#1d5bbd'}}>מים חמים</p>
                <div><p className="text-[24px] md:text-[32px] font-black leading-none text-[#f5a623]">{new Intl.NumberFormat('he-IL', { notation: 'compact', maximumFractionDigits: 0 }).format(records.reduce((sum, r) => sum + (r.specialDebt || 0), 0))}</p></div>
              </div>
        ...
        <div className="w-full space-y-4 md:space-y-6 p-3 md:p-6">
          ...
          <ImportBar lastImportAt={lastImportAt || settings?.last_import_at} isAdmin={isAdmin} />

          {isAdmin &&
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            <button onClick={() => setActiveTab('debtors')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'debtors' ? 'bg-blue-600 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <Users className="w-3.5 h-3.5 flex-shrink-0" /><span>חייבים</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'debtors' ? 'bg-white/25 text-white' : 'bg-blue-100 text-blue-700'}`}>{tabDatasets.debtorsTab.length}</span>
            </button>
            <button onClick={() => setActiveTab('warning')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'warning' ? 'bg-amber-500 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <Mail className="w-3.5 h-3.5 flex-shrink-0" /><span>מכתבי התראה</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'warning' ? 'bg-white/25 text-white' : 'bg-amber-100 text-amber-700'}`}>{tabDatasets.warningTab.length}</span>
            </button>
            <button onClick={() => setActiveTab('legal_candidates')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'legal_candidates' ? 'bg-purple-600 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" /><span>לטיפול משפטי</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'legal_candidates' ? 'bg-white/25 text-white' : 'bg-purple-100 text-purple-700'}`}>{tabDatasets.legalCandidatesTab.length}</span>
            </button>
            <button onClick={() => setActiveTab('legal_process')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'legal_process' ? 'bg-red-600 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <Scale className="w-3.5 h-3.5 flex-shrink-0" /><span>הליך משפטי</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'legal_process' ? 'bg-white/25 text-white' : 'bg-red-100 text-red-700'}`}>{tabDatasets.legalProcessTab.length}</span>
            </button>
            <button onClick={() => setActiveTab('next_actions')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'next_actions' ? 'bg-orange-500 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <CalendarClock className="w-3.5 h-3.5 flex-shrink-0" /><span>פעולות</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'next_actions' ? 'bg-white/25 text-white' : 'bg-orange-100 text-orange-700'}`}>{tabDatasets.nextActionsTab?.length || 0}</span>
            </button>
            <button onClick={() => setActiveTab('archived')} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all ${activeTab === 'archived' ? 'bg-slate-600 text-white' : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'}`}>
              <Archive className="w-3.5 h-3.5 flex-shrink-0" /><span>ארכיון</span>
              <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === 'archived' ? 'bg-white/25 text-white' : 'bg-slate-100 text-slate-600'}`}>{archivedRecords.length}</span>
            </button>
          </div>
          }
