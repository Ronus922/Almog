  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const settings = settingsList[0] || { 
        ...
        threshold_collect_from: 1001,
        threshold_warning_from: 3000, 
      ...
      const calculateDebtStatus = (totalDebt) => {
        ...
        if (totalDebt >= settings.threshold_collect_from && totalDebt < settings.threshold_warning_from) return 'לגבייה';
        if (totalDebt >= settings.threshold_warning_from && totalDebt < settings.threshold_legal_from) return 'מכתב התראה';
        return 'לטיפול משפטי';
      };
