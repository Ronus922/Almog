  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 left-1 w-4 h-4 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center leading-none">
        ...
        <>
          ...
          >
            ...
            <div className="border-t border-slate-100 flex-shrink-0">
              <button
                ...
                className="w-full py-3 text-xs font-medium text-blue-600 hover:bg-blue-50 transition-colors"
              >
                צפה בכל ההתראות
              </button>
            </div>
          ...
          >
            ...
            <div className="flex flex-wrap border-b border-slate-100 bg-white flex-shrink-0">
              {TABS.map(tab => (
                ...
                >
                  {tab.label}
                  {tab.key === 'unread' && unreadCount > 0 && (
                    <span className="mr-1 bg-red-100 text-red-600 rounded-full px-1.5 py-0.5 text-[10px] font-bold">{unreadCount}</span>
                  )}
                </button>
              ))}
            </div>
                ...
                filteredNotifs.map(n => (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span className="text-base mt-0.5 flex-shrink-0">{typeIcon(n.type)}</span>
                      <div className="flex-1 min-w-0">
                        {n.title && <p className={`text-xs font-bold mb-0.5 ${!n.is_read ? 'text-blue-700' : 'text-slate-500'}`}>{n.title}</p>}
                        <p className={`text-sm leading-snug ${!n.is_read ? 'font-semibold text-slate-800' : 'text-slate-600'}`}>{n.message}</p>
                        <p className="text-xs text-slate-400 mt-1">{formatDate(n.created_date)}</p>
                      </div>
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {!n.is_read && <div className="w-2 h-2 bg-blue-500 rounded-full mt-1" />}
                        <button onClick={(e) => deleteNotif(n.id, e)} className="text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
            ...
            <div className="border-t border-slate-100">
              <button onClick={() => { setOpen(false); navigate('/AllNotifications'); }} className="w-full py-2.5 text-xs font-medium text-blue-600 hover:bg-blue-50 transition-colors">
                צפה בכל ההתראות
              </button>
            </div>
          </div>
