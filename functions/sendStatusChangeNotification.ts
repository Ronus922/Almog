...
Deno.serve(async (req) => {
    try {
        ...
        if (!resendApiKey) {
            return Response.json({ error: 'RESEND_API_KEY לא מוגדר - הגדר בהגדרות המערכת' }, { status: 500 });
        }
...
<p style="margin:0;font-size:36px;font-weight:800;color:#dc2626;text-align:center">${formatCurrency(totalDebt)}</p>
</td>
</tr>
</table>
<table cellpadding="0" cellspacing="0" border="0" width="100%">
...
<p style="margin:0;font-size:20px;font-weight:700;color:#ea580c;text-align:center">${formatCurrency(monthlyDebt)}</p>
</td>
<td width="4%"></td>
...
<p style="margin:0;font-size:20px;font-weight:700;color:#9333ea;text-align:center">${formatCurrency(specialDebt)}</p>
</td>
</tr>
</table>
</td>
